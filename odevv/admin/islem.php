...
if (isset($_POST['hakkimizdakaydet'])) {
   ...
   if ($_FILES['resim'] ["size"]>0){
    ...
    $file_name = $_FILES['resim']['name'];
      ...
      $dosya    = $_FILES["resim"]['name']; // dosya ismini aldık
    ...
    $resimadi=$uploads_dir.$sayilar.".".$uzanti;
    move_uploaded_file($file_tmp,$resimadi);
    ...
    $kaydet=$baglanti->prepare("UPDATE hakkimizda SET
       ...
       if($update){
        $eskiresim=$_POST['eskiresimm'];
        unlink($eskiresim);
        
        Header("Location:hakkimizda.php?durum=okey&".$resimadi."");
       }else{
...
$kaydet=$baglanti->prepare("INSERT into menu_listesi SET 
...
if ($insert) {
Header("Location:ekle-menu.php?durum=okey");
}else{
    Header("Location:ekle-menu.php?durum=no");
}
...
if (isset($_POST['menu-guncelle'])) {
    ...
    $kaydet=$baglanti->prepare("UPDATE menu_listesi SET 
    ...
    if ($insert) {
    Header("Location:menu-duzenle.php?id=".$_POST['id']."&durum=okey");
    }else{
        Header("Location:menu-duzenle.php?id=".$_POST['id']."&durum=no");
    }
    
    }

    if (isset($_POST['menu-sil'])) {
    


        $kaydet=$baglanti->prepare("DELETE FROm menu_listesi 
        ...
        Header("Location:menu-liste.php?durum=okey");
